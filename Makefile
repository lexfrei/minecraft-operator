...
IMG ?= ghcr.io/lexfrei/minecraft-operator:latest
...
manifests: controller-gen ## Generate WebhookConfiguration, ClusterRole and CustomResourceDefinition objects.
	$(CONTROLLER_GEN) rbac:roleName=manager-role crd webhook paths="./..." output:crd:dir=charts/minecraft-operator-crds/crds output:rbac:dir=charts/minecraft-operator/templates/rbac
...
templ: ## Generate Go code from templ templates
	...
	templ generate --path pkg/webui/templates
...
KIND_CLUSTER ?= minecraft-operator-test-e2e
...
.PHONY: build
build: manifests generate templ fmt vet ## Build manager binary.
	go build -o bin/manager cmd/main.go

.PHONY: run
run: manifests generate templ fmt vet ## Run a controller from your host.
	go run ./cmd/main.go
...
.PHONY: helm-upgrade
helm-upgrade: manifests ## Upgrade operator via Helm
...
.PHONY: generate-api
generate-api: oapi-codegen ## Generate API types and server from OpenAPI spec.
